	...
	<div class="post-item-content">
		<p>
			<c:if test="${isTopicTrendNotice }">
				...
				<a class="notice-topic-title" href="<c:url value="/topic/${notice.topicId }" />">
				    ${notice.getTopicTitle() }
				</a>
				<span>收到了一条</span>
			...
			<c:if test="${isPostTrendNotice }">
				...
				<a class="notice-post-digest" href="<c:url value="/topic/${notice.getTopicId() }" />">${notice.getPostDigest().subString(0, 10) }</a> 
				<span>收到了一条</span>
				<a href="<c:url value="/topic/${notice.getTopicId() }" />">新回复</a>
		    </c:if>
