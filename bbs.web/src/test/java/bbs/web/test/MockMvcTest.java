...
public class MockMvcTest extends BaseTest {
	...
	public void setup() {
		mvc = MockMvcBuilders.webAppContextSetup(wac)
				.apply(springSecurity()) //SecurityMockMvcConfigurers.springSecurity() will perform all of the initial setup we need to integrate Spring Security with Spring MVC Test
				.build();
	}
	
	@Test
	//@WithUserDetails(value = "varrickt", userDetailsServiceBeanName="userDetailsServiceImp")
	public void testLogin() throws Exception {
		UserDetails ud = this.uds.loadUserByUsername("verrickt");
		mvc
			.perform(get("/").with(user(ud)));
		mvc.perform(logout())
			.andExpect(unauthenticated());
		
		mvc
			.perform(formLogin().user("123").password("invalidpass"))
			.andExpect(unauthenticated());
		
		mvc.perform(formLogin().user("verrickt").password("123456"))
	...
	public void testActionPull() throws Exception {
		...
		MvcResult result = mvc.perform(get("/action/test").with(user(userDetails))
				.param("userId", "1"))
